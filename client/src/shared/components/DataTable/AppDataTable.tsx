...
const AppDataTable: React.FC<AppDataTableProps> = ({ columns, service, params }) => {
    ...
    const fetchData = async () => {
        ...
        products.map((product: any) => {
            product.images = product.images.split(',').map((image: string) => `${services.api_url}/${image}`);
        ...
        <AppDataTableStyle>
                        ...
                        {data.map((row, rowIndex) => (
                          <tr key={rowIndex}>
                            {columns.map((column, colIndex) => (
                                <td key={colIndex} className={column.columnClassName} width="200">
                                    {column.accessor ? 
                                        column.truncate && column.maxChars ? 
                                        row[column.accessor].length > column.maxChars ? 
                                            `${row[column.accessor].slice(0, column.maxChars)}...` : 
                                            row[column.accessor] 
                                        : row[column.accessor] 
                                        : column.Cell ? column.Cell({ value: row }) : null
                                    }
                                </td>
                            ))}
                          </tr>
            ...
            <div className='d-flex align-items-center justify-content-between flex-column flex-md-row gap-3 mt-3'>
                <div className='d-flex align-items-center gap-1 gap-sm-3'>
                    <AppButton onClick={() => handlePageChange(1)} disabled={pagination.page === 1} icon={'angle-double-left'} variant='dark'></AppButton>
                    <AppButton onClick={() => handlePageChange(pagination.page - 1)} disabled={pagination.page === 1} icon={'angle-left'} variant='dark'></AppButton>
                    <div className='d-flex gap-2 gap-sm-4'>
                        {pages.map((page, index) => (
                            <AppButton
                                ...
                                label={`${index + 1}`}
                                className={`py-2 px-3 ${pagination.page === page ? 'text-white' : 'text-dark'}`}
                                variant={pagination.page === page ? 'primary' : 'white'}
                            >
                            </AppButton>
                        ))}
                    </div>
                    <AppButton onClick={() => handlePageChange(pagination.page + 1)} disabled={pagination.page === totalPages} icon={'angle-right'} variant='dark'></AppButton>
                    <AppButton onClick={() => handlePageChange(totalPages)} disabled={pagination.page === totalPages} icon={'angle-double-right'} variant='dark'></AppButton>
                </div>
